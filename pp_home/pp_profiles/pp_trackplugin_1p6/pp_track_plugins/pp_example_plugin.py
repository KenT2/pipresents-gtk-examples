...
I have integrated his idea into Pi Presents such that you can call a plugin from
 imageplayer, videoplayer, messageplayer  and browserplayer.
Plugins work for all types of show but Liveshows and Artliveshows need special treatment.

API
----
Track plugins should be in the profile in the directory /pp_track_plugins or in  /pipresents/pp_track_plugins.
 Plugins are organised as Python Classes.
The class name must be identical to the name of the file that contains it (without .py)
The plugin to be used by a track is specified in the xxxxx.cfg file. The path of the plugin file
 is in the Plugin Config File parameter of a track.
...
Arguments of __init__ :
    root -
    the top level window - not used by GTK4.
    
    canvas -
    the GTK4 Fixed widget  (self.canvas) on which all displayed output of the Player is written.
     A plugin could write to the canvas directly and return the provided file path unchanged (see krt_time.py)
...
------
  ...
  load has the following arguments:
    ...
    False if the plugin is called from any other type of show. These shows have
     medialist and track_file and type are provided by the track entry in the medialist and 
    
    True if the plugin has been called from a liveshow or artliveshow. If called from a
     liveshow the track_file parameter contains rubbish
    and the type paramter is obtained from the .cfg file that specified the plugin.
     This is because the plugin is
    the track, not associated with another type of track. If liveshow is True the load
     method must return the track file (see below)

    b. type
    The type of track, this determines the player that will play the track.
     Currently  mpv, image, webkit, message
    ...
    For all other players - The full path of the file specified in the Location field of
     the profile entry for this track. For audio this may be blank.
...
-----------
    ...
    show() allows user code to  draw on the screen (canvas) while the track is showing. Drawing is by use of GTK4  objects.

    The objects are drawn after other elements of the track are displayed so appear on top. They are displayed immeadiately
     so they should be made visible.
...
redraw(self)
---------------
   redraw allows GTK4 objects created in draw to be updated.(See krt_time.py
   
 hide(self)
 -------------
    The plugin code must  have the method  hide(self)
    It is executed at the end of a track when the display of the track ends.
    The method must hide then any GTK4 objects created by the plugin
    It should also be used to delete any temporary files.
    Note: This method is executed after the show method for the next track. This should not matter
     as each track has its own instance of a plugin.


EXAMPLE
-------------
An example follows. It demonstrates the principles and is very long as it addresses all show and track type combinations.
Usually a plugin will be written for a single track type. krt_time.py and krt_image_text.py are shorter examples 
demonstrating file modification  and drawing .
"""
...
class pp_example_plugin(object):
    ...
    def load(self,track_file,liveshow,track_type):
        ...
        if self.liveshow==True:
            # liveshows have a limited number of track types image, mpv, webkit. Plugins can add message to liveshows
    ...
    def show(self):
        ...
        self.canvas.put(plugin_obj1,100,100)
    ...
    def redraw(self):
        ...
        time_text='My Local Time is: ' + time.asctime()
        # index is 0 as lists start at 0
        self.plugin_objects[0].set_label(time_text)
